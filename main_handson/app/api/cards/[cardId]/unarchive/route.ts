// spec/004_card_movement_archive_restore.md § 操作: カードのアーカイブ復元
...
export async function POST(_req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    const result = await cardRepository.clearState(cardId, "archivedAt");
    if (result.kind === "not_found") return notFound("指定されたカードが見つかりません");
    auditLog(requestId, "card.unarchive", { cardId });
