...
3. `npm test`（フルスイート）→ 既存 16 ファイル・130 tests は全て PASS、新規2ファイルのみ FAIL。既存挙動への影響なし。
...
- Green 確認後、既存 130 tests が壊れていないこと（回帰なし）も合わせて確認する。
...
- 対象2ファイル・11 tests、全18ファイル・141 tests、lint、typecheck、buildがすべて成功した。
...
- 整理後も対象2ファイル・11 tests、全18ファイル・141 tests、lint、typecheck、buildがすべて成功した。
