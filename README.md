...
<repo>/
├── README.md
├── Dockerfile                ← 補助環境。稼働アプリ本体ではない
├── docker-compose.yml        ← 補助環境。稼働アプリ本体ではない
├── docs/
│   └── docker.md
├── minimum_handson/
...
ローカルのNode.jsやnpm環境で詰まる場合に備えて、補助的なDocker構成を用意しています。
標準手順はローカル環境での実行ですが、同じNode.js環境で確認したい場合は以下を参照してください。
