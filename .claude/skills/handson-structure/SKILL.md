...
- 教材の章ごとに**トップレベルのハンズオン用ディレクトリ**を分ける:
  - `minimum_handson/` … 教材 第2章
  - `main_handson/` … 教材 第5章
- **各ハンズオンのディレクトリ直下が「常に最新の稼働アプリ」**。スナップショットはその直下の `1_xxx/`, `2_xxx/`。
...
> ステップ番号は各ハンズオンディレクトリごとに `1_` から始める（ディレクトリ間で通し番号にはしない）。

## 2. 稼働アプリ と スナップショット

各ハンズオンディレクトリ（`minimum_handson/` `main_handson/`）は「稼働アプリ 1つ」＋「ステップのスナップショット複数」で構成する。
